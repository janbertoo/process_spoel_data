...
for vidnumber in vidnumbers:
    ...
    for i in range(len(df) - 1):
        ...
        if current_row['ID'] == next_row['ID'] and abs(current_row['timestamp'] - next_row['timestamp']) <= 0.3:
            ...
            #rotation_log = abs(next_row['log_angle'] - current_row['center_geo_x'])
            ...
            # Parse the log corner coordinates
            if next_row['orientation'] == 'tlbr':
                top_left_next_row = parse_coordinates(next_row['top_left_geo'])
                bottom_right_next_row = parse_coordinates(next_row['bottom_right_geo'])
                if top_left_next_row is not None and bottom_right_next_row is not None:
                    log_vector_next_row = bottom_right_next_row - top_left_next_row
                else:
                    skipped_rows += 1
                    continue  # Skip this row if the coordinates cannot be parsed
            else:
                top_right_next_row = parse_coordinates(next_row['top_right_geo'])
                bottom_left_next_row = parse_coordinates(next_row['bottom_left_geo'])
                if top_right_next_row is not None and bottom_left_next_row is not None:
                    log_vector_next_row = bottom_left_next_row - top_right_next_row
                else:
                    skipped_rows += 1
                    continue  # Skip this row if the coordinates cannot be parsed
            ...
            log_angle_next_row = calculate_angle(log_vector_next_row, flow_vector)

            rotation = np.abs(log_angle - log_angle_next_row)
...
# Analyze the correlation between normalized_velocity and rotation
total_velocity_df = total_velocity_df.dropna(subset=['normalized_velocity', 'rotation'])
...
total_velocity_df = total_velocity_df[total_velocity_df['drone_number'] != 16]

# Pearson correlation
pearson_corr, pearson_p_value = stats.pearsonr(total_velocity_df['rotation'], total_velocity_df['normalized_velocity'])
print(f"Pearson correlation coefficient: {pearson_corr:.3f}, P-value: {pearson_p_value:.3f}")

# Spearman correlation
spearman_corr, spearman_p_value = stats.spearmanr(total_velocity_df['rotation'], total_velocity_df['normalized_velocity'])
print(f"Spearman correlation coefficient: {spearman_corr:.3f}, P-value: {spearman_p_value:.3f}")

# Linear regression
slope, intercept, r_value, p_value, std_err = stats.linregress(total_velocity_df['rotation'], total_velocity_df['normalized_velocity'])

# Plotting
plt.figure(figsize=(6, 4))
plt.scatter(total_velocity_df['log_angle'], total_velocity_df['normalized_velocity'], alpha=0.6)
x_vals = np.array(plt.gca().get_xlim())
y_vals = intercept + slope * x_vals
plt.plot(x_vals, y_vals, color='red', label=f'Trend (Slope: {slope:.3f} \nP-value: {p_value:.3f})')
plt.title('Log Angle vs Normalized Velocity')
plt.xlabel('Rotation (degrees / 0.25s)')
plt.ylabel('Normalized Velocity (m/s)')
plt.ylim(-3.5, 5)
plt.legend()
plt.tight_layout()
plt.savefig('step10_analyze_rotation/analyze_normalized_velo_with_rotation_with_trendline_onlyLarge_nod16.pdf')
plt.savefig('step10_analyze_rotation/analyze_normalized_velo_with_rotation_with_trendline_onlyLarge_nod16.png')
plt.show()
