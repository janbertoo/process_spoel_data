...
def interpolate_velocities(df, new_coordinate):
    ...
    x = float(interpolated_vx)
    if math.isnan(x) == True:
        interpolated_vx = closest_velocities[0, 0]
        interpolated_vy = closest_velocities[0, 1]
...
for vidnumber in [1,2,3,4,5]:
    ...
    x_range = range(int(-20/step_size), int(180/step_size))
    y_range = range(int(50/step_size), int(200/step_size))

    # Create a list to store coordinates
    coordinates = []

    # Generate rotated coordinates using nested loops
    for y in y_range:
        for x in x_range:
            rotated_x, rotated_y = rotate_point(x * step_size, y * step_size, rotation_angle)
            coordinates.append([rotated_x, rotated_y])

    # Print or use rotated_coordinates as needed
    ...
    for coord in coordinates:
    	...
    	if result == True:
    		if coord[0] * m_line1 + b_line1 < coord[1]:
    			...
    			new_coors_and_velocities.append([coord[0],coord[1],newvx,newvy,np.sqrt(newvx**2+newvy**2)])
    ...
    csv_file_path = os.path.join(curdir,'velocities_vid'+str(vidnumber),'all_vid'+str(vidnumber)+'_rotated.csv')

    # Write the list to the CSV file
    with open(csv_file_path, 'w', newline='') as csvfile:
        csv_writer = csv.writer(csvfile)
        csv_writer.writerow(['X','Y','Vx','Vy','Norme'])
