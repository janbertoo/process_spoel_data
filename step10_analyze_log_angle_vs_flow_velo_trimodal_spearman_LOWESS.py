...
from scipy.stats import gaussian_kde, normaltest, shapiro, anderson, spearmanr
...
sns.histplot(total_velocity_df['flow_speed'], bins=30, kde=False)  # Removed KDE smoothing
plt.xlabel('Flow Velocity (m/s)')
plt.ylabel('Frequency')
#plt.title('Histogram of Flow Velocity')
plt.savefig('step10_analyze_rotation/histogram_flow_velocity.png')
plt.savefig('step10_analyze_rotation/histogram_flow_velocity.pdf')
plt.show()
...
# Step 5: Compute Spearman correlation per cluster
for cluster in range(num_clusters):
    subset = total_velocity_df[total_velocity_df['cluster'] == cluster]
    rho, p_value = spearmanr(subset['log_angle'], subset['flow_speed'])
    print(f"Cluster {cluster}: Spearman correlation = {rho:.3f}, p-value = {p_value:.3f}")


# Step 6: Plot LOWESS curve for each cluster
plt.figure(figsize=(6, 4))
colors = ['red', 'blue', 'green']
for cluster in range(num_clusters):
    subset = total_velocity_df[total_velocity_df['cluster'] == cluster]
    X = subset['log_angle'].values
    y = subset['flow_speed'].values
    lowess = sm.nonparametric.lowess(y, X, frac=0.3)
    
    plt.scatter(X, y, alpha=0.2, s=10, label=f'Cluster {cluster}', color=colors[cluster])
    plt.plot(lowess[:, 0], lowess[:, 1], color='black', linestyle='dashed', linewidth=2)

plt.xlabel('Log Angle (degrees)')
plt.ylabel('Flow Velocity (m/s)')
plt.legend()
#plt.title('LOWESS Curve for Each Cluster')
plt.savefig('step10_analyze_rotation/log_angle_vs_flow_speed_trendline_trimodal.png')
plt.savefig('step10_analyze_rotation/log_angle_vs_flow_speed_trendline_trimodal.pdf')
plt.show()
