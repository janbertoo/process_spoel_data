...
weights = 1 / (var1**2)  # Inverse variance as weight
X = sm.add_constant(var1)
wls_model = sm.WLS(var2, X, weights=weights).fit()
...
y_vals = wls_model.params[0] + wls_model.params[1] * x_vals
plt.plot(x_vals, y_vals, color='red', label=f'Trend (WLS)')

plt.xlabel('Length (m)')
plt.ylabel('Rotation (° / 0.25 s)')
plt.legend()
plt.tight_layout()
plt.savefig('step10_analyze_rotation/analyze_length_vs_rotation_with_WLS.png')
plt.savefig('step10_analyze_rotation/analyze_length_vs_rotation_with_WLS.pdf')
