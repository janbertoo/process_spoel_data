    ...
    for folder in drone_vid_list:
        ...
        for imfile in os.listdir(imagespath):
            if imfile[-4:] == '.jpg' and int(imfile[-9:-4]) % (fps/freq) == 0:
                #print(imfile[-9:-4])
                #print(mainpath)
                hompath = os.path.join(mainpath,'DCIM-drone'+str(dronenumber),'drone'+str(dronenumber)+'vid'+str(vidnumber),'homs_remove1_19jan')
...
for vidnumber in [3,4]:
    outputdir = os.path.join(curdir,'vid_19jan_'+str(vidnumber))
    #print(outputdir)
    ...
    for i in range(0,20000):
        ...
        output_frame_name = os.path.join(outputdir,'frame'+str(framenumber).zfill(5)+'.jpg')

        try:
            drone1_output_hom_name = output_frame_name.replace('.jpg',drone1hom.split('/')[-1])
        except:
            drone1_output_hom_name = None
        try:
            drone6_output_hom_name = output_frame_name.replace('.jpg',drone6hom.split('/')[-1])
        except:
            drone6_output_hom_name = None
        try:
            drone16_output_hom_name = output_frame_name.replace('.jpg',drone16hom.split('/')[-1])
        except:
            drone16_output_hom_name = None

        stagered_drone_ims.append([[drone1im,drone6im,drone16im],[drone1hom,drone6hom,drone16hom],output_frame_name,[drone1_output_hom_name,drone6_output_hom_name,drone16_output_hom_name],outputdir])
    ...
    try:
        ...
        if framenumber % 8 == 0:
            ...
            if im3 == None:
                ...
                #hom3 = hom3.replace('_hom.p','(28.063999999780208,_64.52499999990687,_-0.6960116249807768)_hom.p') #slecht
                
                #hom3 = hom3.replace('_hom.p','(48.69799999985844,_68.66800000006333,_0.42687074919507306)_hom.p') #slecht

                
                #hom3 = hom3.replace('_hom.p','(13.959999999962747,_60.59499999997206,_3.6053464516424887)_hom.p') #ok

                
                hom3 = hom3.replace('_hom.p','(17.387000000104308,_58.29399999999441,_1.4626516031982124)_hom.p') #
                ...
                plt.tight_layout()
                #print('hoi3')
                plt.savefig(os.path.join(data[4],data[0][2].split('/')[-1]))
                
                file_1 = open( (os.path.join(data[4],data[0][2].split('/')[-1])).replace('.jpg','_hom.p') ,'wb')
                print(file_1)
                pickle.dump(drone3_homography,file_1)
