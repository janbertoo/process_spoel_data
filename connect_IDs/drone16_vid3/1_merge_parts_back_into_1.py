...
csv_file_path = 'drone16_vid3.csv'
dronenumber = 16
vidnumber = 3
duration = 28166
...
def merge_duplicate_rows(file_path):
    ...
    if not df_duplicates.empty:
        ...
        # Create a new DataFrame for merged rows
        merged_rows = []
        for id, group in df_duplicates.groupby(6):
            min_x = group[1].min()
            min_y = group[2].min()
            max_x = group[1].max() + group[3].max() / 2  # Adjust for width
            max_y = group[2].max() + group[4].max() / 2  # Adjust for height
            for index, row in group.iterrows():
                min_x = min(min_x, row[1] - row[3] / 2)
                min_y = min(min_y, row[2] - row[4] / 2)
                max_x = max(max_x, row[1] + row[3] / 2)
                max_y = max(max_y, row[2] + row[4] / 2)
            width = max_x - min_x
            height = max_y - min_y
            confidence = group[5].max()  # Take the maximum confidence
            merged_rows.append([0, min_x + width / 2, min_y + height / 2, width, height, confidence, id])
