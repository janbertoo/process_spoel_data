...
plt.scatter(total_rotation_df['closest_gradient'], total_rotation_df['abs_rotation'], color='tab:blue', alpha=0.6, label='Data points')

# Overlay the average line
plt.plot(average_rotations['velocity_bins'].cat.codes * 0.5 + 0.25, average_rotations['abs_rotation'], color='red', linewidth=2, label='Average per Bin')

# Titles and labels
plt.title('Scatter plot of Velocity vs Rotation')
plt.xlabel('LSPI Velocity (m/s)')
plt.ylabel('Rotation (° / 0.25 s)')
plt.yticks(np.arange(0, max(total_rotation_df['abs_rotation']) + 1, 1))  # Adjust Y-ticks as necessary
plt.ylim(0, 20)  # Set X-axis limit to a maximum of 30 m/s
plt.legend()
plt.tight_layout()

# Save and show plot
plt.savefig('step10_analyze_rotation/scatter_plot_velocity_vs_rotation.png')
plt.show()
