...
# Define diameter bins (0.05 m increments) and categorize logs
bin_size = 0.1
max_diameter = total_velocity_df['diameter_median'].max()
bin_edges = np.arange(0, max_diameter + bin_size, bin_size)
bin_labels = [f"{bin_edges[i]:.2f} - {bin_edges[i+1]:.2f} m" for i in range(len(bin_edges) - 1)]
total_velocity_df['diameter_bin'] = pd.cut(total_velocity_df['diameter_median'], bins=bin_edges, labels=bin_labels, right=False)

# Plotting the PDF for velocity_log by diameter_bin
plt.figure(figsize=(10, 6))

# Iterate over each diameter bin and plot its PDF in a different color
for label in bin_labels:
    bin_data = total_velocity_df[total_velocity_df['diameter_bin'] == label]
    if not bin_data.empty:
        sns.kdeplot(bin_data['velocity_log'], fill=True, label=label, bw_adjust=0.5)

plt.title('PDF of Log Velocities by Diameter Bins')
plt.xlabel('Log Velocity (m/s)')
plt.ylabel('Density')
plt.xlim(0, 8)
plt.legend(title='Diameter Bins')
plt.tight_layout()
plt.savefig('step9_dataframe/velocity_log_pdf_by_diameter_bins.png')
plt.savefig('step9_dataframe/velocity_log_pdf_by_diameter_bins.pdf')
plt.show()
