...
total_rotation_df = total_rotation_df[total_rotation_df['diameter_median'] >= 0.10]
total_rotation_df = total_rotation_df[total_rotation_df['length_median'] >= 1]

var1 = total_rotation_df['length_median']
var2 = total_rotation_df['abs_rotation']

var1 = pd.to_numeric(var1, errors='coerce')
var2 = pd.to_numeric(var2, errors='coerce')

total_rotation_df = total_rotation_df.dropna(subset=['length_median', 'abs_rotation'])

quantiles = [0.25, 0.5, 0.75]
plt.figure(figsize=(4, 4))
plt.scatter(var1, var2, label='Data points', alpha=0.5, color='tab:blue', s=10)  # Background circles in blue
x_vals = np.linspace(var1.min(), var1.max(), 100)
colors = ['red', 'orange', 'green']  # First quantile is now red

for i, q in enumerate(quantiles):
    quantile_model = smf.quantreg("abs_rotation ~ length_median", total_rotation_df).fit(q=q)
    print(f"Quantile {q} Regression Results:")
    print(quantile_model.summary())
    y_vals = quantile_model.params[0] + quantile_model.params[1] * x_vals
    plt.plot(x_vals, y_vals, color=colors[i], label=f'Quantile {q} (Slope: {quantile_model.params[1]:.3f})')

plt.xlabel('Length (m)')
plt.ylabel('Wood Piece Rotation (° / 0.25 s)')
plt.legend()
#plt.title('Quantile Regression of Rotation vs. Log Length')
plt.tight_layout()
plt.savefig('step10_analyze_rotation/analyze_length_vs_rotation_with_multiple_quantiles.png')
plt.savefig('step10_analyze_rotation/analyze_length_vs_rotation_with_multiple_quantiles.pdf')
