...
water_levels_base = [1490.74, 1490.26, 1489.66, 1489.37, 1488.91, 1488.59, ]
...
for section, section_number, water_level_base, pre_section_file, post_section_file in sections_and_water_levels:
    ...
    fig, axs = plt.subplots(num_subplots, 1, figsize=(4, 2.4 * num_subplots), sharex=True)
    ...
    for i, (correction, vidnumber, color) in enumerate(corrections_and_vidnumber):
        ...
        ax2.axvline(x=weighted_average, color='black', linestyle='--', label='Average',linewidth=2)
    ...
    plt.tight_layout()
    #plt.subplots_adjust(top=0.3, wspace=0.3)  # Adjust the top and wspace as needed
    #plt.show()
    plt.savefig(savefigfolder+'section_'+str(section_number)+'_wood_distribution_per_video.eps')
    plt.savefig(savefigfolder+'section_'+str(section_number)+'_wood_distribution_per_video.png')
    plt.savefig(savefigfolder+'section_'+str(section_number)+'_wood_distribution_per_video.pdf')

for section, section_number, water_level_base, pre_section_file, post_section_file in sections_and_water_levels:
    print('')
    print('Section: ' + str(section))
   
    # Process pre-section and post-section data
    ...
    # Define colors for each video and size category
    size_colors = {
        # For each color, create a light and dark variant
        'red': ['#ffcccc', '#cc0000'],       # Light red, Dark red
        'purple': ['#e6ccff', '#6600cc'],    # Light purple, Dark purple
        'orange': ['#ffe0b3', '#cc7a00'],    # Light orange, Dark orange
        'cyan': ['#ccffff', '#00b3b3'],      # Light cyan, Dark cyan
        'b': ['#cce6ff', '#0066cc']          # Light blue, Dark blue
    }
   
    for i, (correction, vidnumber, base_color) in enumerate(corrections_and_vidnumber):
        df_vid = df[df['vidnumber'] == vidnumber]
        df_vid['line'] = df_vid.apply(create_line, axis=1)
        df_vid['line_intersect'] = df_vid['line'].apply(lambda x: line_intersection(x, section))
        df_vid['line_intersect_distance_from_base'] = df_vid['line_intersect'].apply(lambda x: calc_distance_to_begin_section(x, section))
        ...
        # Separate the wood pieces into two categories based on length
        df_vid_short = df_vid[df_vid['length'] < 1]  # Pieces shorter than 1 meter
        df_vid_long = df_vid[df_vid['length'] >= 1]  # Pieces 1 meter or longer
       
        # Calculate weighted averages for all pieces
        weighted_centers = df_vid['line_intersect_distance_from_base'] * df_vid['volume_median']
        total_volume = df_vid['volume_median'].sum()
        weighted_average = weighted_centers.sum() / total_volume if total_volume > 0 else 0
       
        ax2 = axs[i + 1]
       
        # Create bin edges for histograms
        max_distance = int(df_vid['line_intersect_distance_from_base'].max()) + 2
        bin_edges = np.arange(0, max_distance, size_of_bins)
        bin_centers = (bin_edges[:-1] + bin_edges[1:]) / 2
       
        # Calculate volume sums for short pieces
        volume_sum_short = []
        for j in range(len(bin_edges) - 1):
            mask = (df_vid_short['line_intersect_distance_from_base'] >= bin_edges[j]) & \
                   (df_vid_short['line_intersect_distance_from_base'] < bin_edges[j + 1])
            volume = df_vid_short.loc[mask, 'volume_median'].sum()
            volume_sum_short.append(volume)
       
        # Calculate volume sums for long pieces
        volume_sum_long = []
        for j in range(len(bin_edges) - 1):
            mask = (df_vid_long['line_intersect_distance_from_base'] >= bin_edges[j]) & \
                   (df_vid_long['line_intersect_distance_from_base'] < bin_edges[j + 1])
            volume = df_vid_long.loc[mask, 'volume_median'].sum()
            volume_sum_long.append(volume)
       
        # Get light and dark colors for this video
        light_color = size_colors[base_color][0]
        dark_color = size_colors[base_color][1]
       
        # Plot the bars for both categories
        # For shorter pieces, use light color
        ax2.bar(bin_centers, volume_sum_short, align='center', width=size_of_bins,
                color=light_color, label='Length < 1m', zorder=10)
       
        # For longer pieces, use darker color and stack on top
        ax2.bar(bin_centers, volume_sum_long, align='center', width=size_of_bins,
                bottom=volume_sum_short, color=dark_color, label='Length ≥ 1m', zorder=20)
       
        # Add weighted average line
        ax2.axvline(x=weighted_average, color='black', linestyle='--', label='Average', linewidth=2)
       
        # Calculate total volumes for short and long pieces
        total_volume_short = np.sum(volume_sum_short)
        total_volume_long = np.sum(volume_sum_long)
       
        ax2.set_ylabel('Volume (m³)')
        ax2.legend()
        ax2.set_title(f'Video {vidnumber}, Total Volume: {round(total_volume, 2)} m³\n' +
                      f'Short: {round(total_volume_short, 2)} m³, Long: {round(total_volume_long, 2)} m³')
       
        # Update max_y_value if necessary
        max_volume = max(np.array(volume_sum_short) + np.array(volume_sum_long))
        if max_volume > max_y_value:
            max_y_value = max_volume
   
    # Set the y-axis limit for all additional subplots
    for ax2 in axs[1:]:
        ax2.set_ylim(0, max_y_value * 1.1)  # Add 10% padding
    ...
    for correction, vidnumber, color in corrections_and_vidnumber:
        ...
        ax.axvline(x=weighted_average, color=color, linestyle='--', label=f'Video {vidnumber}', linewidth=2)
   
    # Fix legend in main plot to avoid duplicates
    handles, labels = ax.get_legend_handles_labels()
    by_label = dict(zip(labels, handles))
    ax.legend(by_label.values(), by_label.keys())
   
    plt.tight_layout()
    plt.savefig(savefigfolder+'section_'+str(section_number)+'_wood_distribution_per_video_bins.eps')
    plt.savefig(savefigfolder+'section_'+str(section_number)+'_wood_distribution_per_video_bins.png')
    plt.savefig(savefigfolder+'section_'+str(section_number)+'_wood_distribution_per_video_bins.pdf')
