...
total_velocity_df = total_velocity_df.dropna(subset=['normalised_velocity', 'length_median'])
...
# Pearson correlation
pearson_corr, pearson_p_value = stats.pearsonr(total_velocity_df['length_median'], total_velocity_df['normalised_velocity'])
print(f"Pearson correlation coefficient: {pearson_corr:.3f}, P-value: {pearson_p_value:.3f}")

# Spearman correlation
spearman_corr, spearman_p_value = stats.spearmanr(total_velocity_df['length_median'], total_velocity_df['normalised_velocity'])
print(f"Spearman correlation coefficient: {spearman_corr:.3f}, P-value: {spearman_p_value:.3f}")

# Linear regression
slope, intercept, r_value, p_value, std_err = stats.linregress(total_velocity_df['length_median'], total_velocity_df['normalised_velocity'])

# Plotting
plt.figure(figsize=(6, 4))
plt.scatter(total_velocity_df['length_median'], total_velocity_df['normalised_velocity'], alpha=0.6)
x_vals = np.array(plt.gca().get_xlim())
y_vals = intercept + slope * x_vals
plt.plot(x_vals, y_vals, color='red', label=f'Trend (Slope: {slope:.3f} \nP-value: {p_value:.3f})')
plt.title('Length Median vs Normalised Velocity')
plt.xlabel('Length Median (m)')
plt.ylabel('Normalised Velocity (m/s)')
plt.ylim(-3.5, 5)
plt.legend()
plt.tight_layout()
plt.savefig('step10_analyze_rotation/analyze_normalized_velo_with_length_with_trendline_onlyLarge_nod16.pdf')
plt.savefig('step10_analyze_rotation/analyze_normalized_velo_with_length_with_trendline_onlyLarge_nod16.png')
plt.show()
