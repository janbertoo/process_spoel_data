...
#for vidnumber in [3]:
    ...
    homography_path = labels_with_gcps_path.replace('labels_with_gcps','homs')
    ...
    for entry in all_info:
        ...
        count = 0
        diff_and_homs.append([totaldiff,count,drones_used,virtualGCPS_combos,(homography)])
        count = count + 1
        ...
        for i in range(len(coorstxt)):
            ...
            diff_and_homs.append([op_totaldiff,count,drones_used,virtualGCPS_combos,(op_homography)])
            count = count + 1
        ...
        drones_used = []
        #the best hom so far
        used_drones = diff_and_homs[0][2]
        '''
        #again, remove 1 gcp (as it  might be bad) and instead add a random virtual GCP
        used_drones_minus_one = []
        for q in range(len(used_drones)):
            for w in range(len(used_drone)):
                if q != w:
                    used_drones_minus_one.append(used_drones[w])
        '''

        #now add virtual GCPS
        virtualGCPS_combos = []
        for a in used_drones:
            for s in used_drones:
                if a != s:
                    virtualGCP = create_a_virtual_gcp_from_2_gcps(coorstxt[a],coorstxt[s])
                    virtualGCPS_combos.append([a,s])

                    xyzs = []
                    for j in used_drones:
                        xyzs.append(coorstxt[j][1])
                        drones_used.append(j)
                    #XYZ = np.array(xyzs, dtype=np.float32)
                    ...
                    uvs.append(virtualGCP[0])
                    xyzs.append(virtualGCP[1])
