...
for section, section_number, water_level_base, pre_section_file, post_section_file in sections_and_water_levels:
    ...
    # Transform that maps data coordinates of ax2 to data coordinates of ax
    transform = transforms.blended_transform_factory(ax.transData, ax.transData)

    # Define bin edges once for consistency
    max_distance = max(df_pre_section['distance_from_base'].max(), df_post_section['distance_from_base'].max())
    bin_edges = np.arange(0, int(max_distance) + 2, size_of_bins)
    cumulative_heights = np.zeros(len(bin_edges) - 1)
    ...
    for correction, vidnumber, color in corrections_and_vidnumber:
        ...
        # Bin the 'line_intersect_from_base' values using predefined bin edges
        ...
        cumulative_heights += volume_sum
        
        # Adjust the bottom of the bars to align with y=0 of ax1
        bars = ax2.bar(bin_centers, volume_sum, align='center', width=size_of_bins, alpha=0.5, color=color, bottom=cumulative_heights - volume_sum, label='dingen')
        for bar in bars:
    ...
    ax.set_ylabel('Z (m)')
    ax2.set_ylabel('Total volume per bin (m^3)')
    ax.legend()
    plt.show()
