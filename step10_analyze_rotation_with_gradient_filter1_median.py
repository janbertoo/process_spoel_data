...
for vidnumber in vidnumbers:
    ...
    gradient_df = pd.read_csv('step6_LSPIV/median_speeds_gradients.csv', names=['X', 'Y', 'gradient'])
...
plt.scatter(var1, var2, label='Data points')
plt.plot(var1, trendline, color='red', label=f'Trendline (p={p_value:.3f})')

# Add labels, title, and legend
plt.title('Scatter plot of Rotation vs Gradient with Trendline')
plt.xlabel('Rotation (° / 0.25 s)')
plt.ylabel('Gradient')
plt.legend()

plt.savefig('step10_analyze_rotation/analyze_rotation_vs_gradient_median.pdf')
plt.savefig('step10_analyze_rotation/analyze_rotation_vs_gradient_median.png')
