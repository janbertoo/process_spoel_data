...
merged_df = merged_df.dropna(subset=['acceleration', 'length_median'])

# Classify logs into accelerating and decelerating
merged_df['acceleration_class'] = np.where(merged_df['acceleration'] > 0, 'Accelerating', 'Decelerating')

# Plot KDEs of length_median for both classes
plt.figure(figsize=(8, 5))
sns.kdeplot(
    data=merged_df[merged_df['acceleration_class'] == 'Accelerating'],
    x='length_median',
    fill=True,
    label='Accelerating',
    alpha=0.5
...
sns.kdeplot(
    data=merged_df[merged_df['acceleration_class'] == 'Decelerating'],
    x='length_median',
    fill=True,
    label='Decelerating',
    alpha=0.5
...
plt.title('KDE of Length Median for Accelerating and Decelerating Logs')
plt.xlabel('Length Median')
plt.ylabel('Density')
plt.legend()
plt.tight_layout()

# Save and display the plot
plt.savefig('kde_length_median_acceleration_classes.pdf')
plt.savefig('kde_length_median_acceleration_classes.png')
plt.show()
