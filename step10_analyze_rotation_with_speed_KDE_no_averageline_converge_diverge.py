...
# Example coordinates for polygons (replace with actual coordinates from your data)
...
# Function to classify rows based on polygons
def classify_area(center_geo):
    point = Point(center_geo)  # Create a shapely Point object
    if polygon1.contains(point) or polygon2.contains(point):
        return "diverging"
    else:
        return "converging"

# Assuming 'center_between_geo' is a tuple column in the format (x, y)
total_rotation_df['area_pattern'] = total_rotation_df['center_between_geo'].apply(classify_area)

print(total_rotation_df)

total_rotation_df = total_rotation_df[total_rotation_df['area_pattern'] == 'converging']
...
bins = np.arange(0, total_rotation_df['closest_gradient'].max() + 0.5, 0.5)
labels = [f"{i}-{i + 0.5}" for i in bins[:-1]]
total_rotation_df['velocity_bins'] = pd.cut(total_rotation_df['closest_gradient'], bins=bins, labels=labels, right=False)

# Calculate averages per bin
average_rotations = total_rotation_df.groupby('velocity_bins')['abs_rotation'].mean().reset_index()

# Create a KDE plot instead of a scatter plot
...
plt.plot(average_rotations['velocity_bins'].cat.codes * 0.5 + 0.25, average_rotations['abs_rotation'], color='red', linewidth=2, label='Average Rotation per Bin')

# Titles and labels
plt.title('KDE Plot of Velocity vs Rotation')
plt.xlabel('LSPI Velocity (m/s)')
plt.ylabel('Rotation (° / 0.25 s)')
plt.xlim(0, 6)  # Set X-axis limit to a maximum of 30 m/s
plt.ylim(0, 10)  # Set X-axis limit to a maximum of 30 m/s
#plt.legend()
plt.tight_layout()

# Save and show plot
plt.savefig('step10_analyze_rotation/kde_plot_velocity_vs_rotation_no_averageline_averageline_converging.png')
plt.savefig('step10_analyze_rotation/kde_plot_velocity_vs_rotation_no_averageline_averageline_converging.pdf')
plt.show()
