...
for vidnumber in vidnumbers:
    ...
    total_df = filtered_df_below_1sec_time_diff
    ...
    # Filter the dataframe based on the condition
    smaller_df = total_df[total_df['diameter_median'] > 0.1]
    # Display the new, smaller dataframe
    #print(smaller_df)
    total_df = smaller_df

    # Filter the dataframe based on the condition
    smaller_df = total_df[total_df['length_median'] > 1]
    ...
    print(total_df)
    # Filter the dataframe based on the condition
    #smaller_df = total_df[total_df['volume_median'] > 0.08]
    ...
    total_df['length_median'] = pd.to_numeric(total_df['length_median'], errors='coerce')
    total_df['LSPIV_speed_minus_speed'] = pd.to_numeric(total_df['LSPIV_speed_minus_speed'], errors='coerce')

    # Remove rows with NaN or infinite values
    total_df = total_df.replace([np.inf, -np.inf], np.nan)
    total_df.dropna(subset=['length_median', 'LSPIV_speed_minus_speed'], inplace=True)

    # Create a figure and axis object
    fig, ax = plt.subplots(figsize=(20,10))

    # Plot 'speed' column as dots
    ax.scatter(total_df['length_median'], total_df['LSPIV_speed_minus_speed'], label='Speed', marker='.')

    # Perform linear regression only on non-NaN values
    x = total_df['length_median']
    y = total_df['LSPIV_speed_minus_speed']
    ...
    # Convert columns to numeric data type
    total_df['diameter_median'] = pd.to_numeric(total_df['diameter_median'], errors='coerce')
    total_df['LSPIV_speed_minus_speed'] = pd.to_numeric(total_df['LSPIV_speed_minus_speed'], errors='coerce')

    # Remove rows with NaN or infinite values
    total_df = total_df.replace([np.inf, -np.inf], np.nan)
    total_df.dropna(subset=['diameter_median', 'LSPIV_speed_minus_speed'], inplace=True)

    # Create a figure and axis object
    fig, ax = plt.subplots(figsize=(20,10))

    # Plot 'speed' column as dots
    ax.scatter(total_df['diameter_median'], total_df['LSPIV_speed_minus_speed'], label='Speed', marker='.')

    # Perform linear regression only on non-NaN values
    x = total_df['diameter_median']
    y = total_df['LSPIV_speed_minus_speed']
    ...
    # Convert columns to numeric data type
    total_df['volume_median'] = pd.to_numeric(total_df['volume_median'], errors='coerce')
    total_df['LSPIV_speed_minus_speed'] = pd.to_numeric(total_df['LSPIV_speed_minus_speed'], errors='coerce')

    # Remove rows with NaN or infinite values
    total_df = total_df.replace([np.inf, -np.inf], np.nan)
    total_df.dropna(subset=['volume_median', 'LSPIV_speed_minus_speed'], inplace=True)

    # Create a figure and axis object
    fig, ax = plt.subplots(figsize=(20,10))

    # Plot 'speed' column as dots
    ax.scatter(total_df['volume_median'], total_df['LSPIV_speed_minus_speed'], label='Speed', marker='.')

    # Perform linear regression only on non-NaN values
    x = total_df['volume_median']
    y = total_df['LSPIV_speed_minus_speed']
    ...
    # Convert columns to numeric data type
    total_df['volume_median'] = pd.to_numeric(total_df['volume_median'], errors='coerce')
    total_df['speed'] = pd.to_numeric(total_df['speed'], errors='coerce')

    # Remove rows with NaN or infinite values
    total_df = total_df.replace([np.inf, -np.inf], np.nan)
    total_df.dropna(subset=['volume_median', 'speed'], inplace=True)

    # Create a figure and axis object
    fig, ax = plt.subplots(figsize=(20,10))

    # Plot 'speed' column as dots
    ax.scatter(total_df['volume_median'], total_df['speed'], label='Speed', marker='.')
    ...
    # Filter the dataframe based on the condition
    smaller_df = total_df[total_df['volume_median'] > 0.5]
    print(smaller_df)