...
# Define 0.25-meter size classes
bin_size = 0.25
max_length = total_rotation_df['length_median'].max()
bin_edges = np.arange(0, max_length + bin_size, bin_size)
bin_labels = [f"{bin_edges[i]:.2f} - {bin_edges[i+1]:.2f} m" for i in range(len(bin_edges) - 1)]
total_rotation_df['length_class'] = pd.cut(total_rotation_df['length_median'], bins=bin_edges, labels=bin_labels, right=False)

# Filter out the first 4 bins with no data
filtered_rotation_df = total_rotation_df[total_rotation_df['length_class'].isin(bin_labels[4:])]

# Reassign the `length_class` as categorical to avoid empty bins in the plot
filtered_rotation_df['length_class'] = pd.Categorical(filtered_rotation_df['length_class'], categories=bin_labels[4:], ordered=True)

# Create boxplots of rotation by 0.25-meter length class using the filtered dataframe
plt.figure(figsize=(12, 8))
filtered_rotation_df.boxplot(column='rotation', by='length_class', grid=False)
plt.title('Rotation by Wood Size Classes (0.25 m Increments)')
plt.suptitle('')  # Removes the automatic title
plt.xlabel('Wood Size Classes (Length)')
plt.ylabel('Rotation (° / 0.25 s)')
plt.ylim(0, 10)  # Set the y-axis limit to 10
...
plt.savefig('step10_analyze_rotation/rotation_by_025m_size_classes_boxplot_right_aligned.png')
plt.savefig('step10_analyze_rotation/rotation_by_025m_size_classes_boxplot_right_aligned.pdf')
plt.show()
