...
# Quantile Regression
quantile_model = smf.quantreg("abs_rotation ~ length_median", total_rotation_df).fit(q=0.5)  # Median regression
...
plt.plot(x_vals, y_vals, color='red', label=f'Trend (Quantile Regression)')

plt.xlabel('Length (m)')
plt.ylabel('Rotation (° / 0.25 s)')
plt.legend()
plt.tight_layout()
plt.savefig('step10_analyze_rotation/analyze_length_vs_rotation_with_quantile_regression.png')
plt.savefig('step10_analyze_rotation/analyze_length_vs_rotation_with_quantile_regression.pdf')
