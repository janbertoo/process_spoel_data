...
for section, section_number, water_level_base, pre_section_file, post_section_file in sections_and_water_levels:
    ...
    #df_pre_section = df_pre_section.sort_values(by='distance_from_base')
    #print(df_pre_section)
    #Correcting an issue I had with a rectracting GPS pole on pre-section-day
    if section_number == 1:
        print(section_number)
        df_pre_section['Z'] = df_pre_section['Z'] + 0.245 - ( ( 0.245 + 0.273 ) / 45 ) * df_pre_section['distance_from_base']
    print(df_pre_section)
    df_pre_section.sort_values('distance_from_base')
    ...
    #df_post_section = df_post_section.sort_values(by='distance_from_base')
    df_post_section['Z'] = df_post_section['Z'] - water_level_base
    print(df_post_section)
    ...
    ax.plot(df_pre_section['distance_from_base'], df_pre_section['Z'], color='blue', label='Pre Section')
    ax.plot(df_post_section['distance_from_base'], df_post_section['Z'], color='red', label='Post Section')
    ...
    ax2 = ax.twinx()
    # Define the value where you want the secondary y-axis to start
    start_value = 0  # Adjust as needed

    # Set the limits of ax2 to start from start_value
    ax2.set_ylim(start_value, ax.get_ylim()[1])  # Ensure that the upper limit is the same as ax1

    #print(df_post_section['distance_from_base'])
    #print(df_post_section['Z'])
    
    for correction,vidnumber,color in corrections_and_vidnumber:
        df_vid = df[df['vidnumber'] == vidnumber]
        #df_vid['line'] = ( ( df_vid['center_geo'][0],df_vid['center_geo'][1] ), ( df_vid['center_before'][0], df_vid['center_before'][1] ) )
        df_vid['line'] = df_vid.apply(create_line, axis=1)
        #df_vid['intersect'] = line_intersection(section,df_vid['line'])
        ...
        # Calculate the weighted center for each data point
        ...
        plt.axvline(x=weighted_average, color=color, linestyle='--', label='Weighted Average')

        # Define bin edges
        bin_edges = np.arange(0, int(df_vid['line_intersect_distance_from_base'].max()) + 2, size_of_bins)

        # Bin the 'line_intersect_from_base' values
        binned_values, _ = np.histogram(df_vid['line_intersect_distance_from_base'], bins=bin_edges)

        # Calculate bin centers
        bin_centers = (bin_edges[:-1] + bin_edges[1:]) / 2

        # Calculate corresponding 'volume_media' sum for each bin
        volume_sum = []
        for i in range(len(bin_edges) - 1):
            mask = (df_vid['line_intersect_distance_from_base'] >= bin_edges[i]) & (df_vid['line_intersect_distance_from_base'] < bin_edges[i + 1])
            volume = df_vid.loc[mask, 'volume_median'].sum()
            volume_sum.append(volume)

        # Plot the binned data
        ax2.bar(bin_centers, volume_sum, align='center', width=size_of_bins, alpha=0.5, color=color)

        # Plot vertical lines at the bin centers
        #for center in bin_centers:
        #    ax2.axvline(x=center, color='r', linestyle='--')


        print('Vid number: '+str(vidnumber))        
        water_level = correction
        print('Water level: '+str(water_level))

    
    
    plt.title('Section '+str(section_number))
    #plt.tight_layout()
    ax.set_xlabel('Distance from North Bank (m)')
    ax.set_ylabel('Z (m)')
    ax.legend()
    #plt.show()
    plt.savefig(savefigfolder+'section_'+str(section_number)+'wood_distr_opacity.png')
'''
