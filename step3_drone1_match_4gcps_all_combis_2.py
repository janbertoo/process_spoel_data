...
curdir = os.getcwd()
#print(curdir)
csvdata = os.path.join(curdir,'GPS_Data-20230619T123957Z-001/GPS_Data/drone1_spoelgcps_notest.csv')#_plus_virtual.csv')
#print(csvdata)
#csvdataforprocessing = os.path.join(curdir,'GPS_Data-20230619T123957Z-001/GPS_Data/drone1_spoelgcps_notest.csv')
csvdataforprocessing = csvdata

dronepath = os.path.join(curdir,'cut_data','DCIM-drone1')
...
def find_all_gcps_in_text_file(lookup_table,txt_file):
    ...
    for i in range(len(combinationshad)):
        for j in range(len(combinationshad[i])):
            ...
            if key_for_gcp[i][j] == 4:
                print('FOUND KEY 4 FOR: ',txt_file)
