...
var1 = total_rotation_df['diameter_median']
var2 = total_rotation_df['abs_rotation']

var1 = pd.to_numeric(var1, errors='coerce')
var2 = pd.to_numeric(var2, errors='coerce')

total_rotation_df = total_rotation_df.dropna(subset=['diameter_median', 'abs_rotation'])

# White's Test for heteroscedasticity
X = sm.add_constant(var1)
model = sm.OLS(var2, X).fit()
...
plt.scatter(var1, var2, label='Data points', alpha=0.5, color='tab:blue', s=10)  # Background circles in tab:blue
x_vals = np.linspace(var1.min(), var1.max(), 100)
colors = ['red', 'orange', 'green']  # First quantile is now red

for i, q in enumerate(quantiles):
    quantile_model = smf.quantreg("abs_rotation ~ diameter_median", total_rotation_df).fit(q=q)
...
plt.legend()
plt.title('Quantile Regression of Rotation vs. Log Diameter')
plt.tight_layout()
plt.savefig('step10_analyze_rotation/analyze_diameter_vs_rotation_with_multiple_quantiles.png')
plt.savefig('step10_analyze_rotation/analyze_diameter_vs_rotation_with_multiple_quantiles.pdf')
