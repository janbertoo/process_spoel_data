...
for vidnumber in vidnumbers:
    ...
    for i in range(len(df) - 1):
        ...
        if current_row['ID'] == next_row['ID'] and abs(current_row['timestamp'] - next_row['timestamp']) <= 0.3:
            velocity_log = ( np.sqrt((next_row['center_geo_x'] - current_row['center_geo_x']) ** 2 + (next_row['center_geo_y'] - current_row['center_geo_y']) ** 2) ) / 0.25
...
# Analyze the correlation between velocity_log and log_angle
total_velocity_df = total_velocity_df.dropna(subset=['velocity_log', 'log_angle'])
...
# Pearson correlation
pearson_corr, pearson_p_value = stats.pearsonr(total_velocity_df['log_angle'], total_velocity_df['velocity_log'])
...
# Linear regression
slope, intercept, r_value, p_value, std_err = stats.linregress(total_velocity_df['log_angle'], total_velocity_df['velocity_log'])

# Plotting
plt.figure(figsize=(6, 4))
plt.scatter(total_velocity_df['log_angle'], total_velocity_df['velocity_log'], alpha=0.6)
x_vals = np.array(plt.gca().get_xlim())
y_vals = intercept + slope * x_vals
plt.plot(x_vals, y_vals, color='red', label=f'Trend (Slope: {slope:.3f} \nP-value: {p_value:.3f})')
plt.title('Log Angle vs Log Velocity')
plt.xlabel('Log Angle (degrees)')
plt.ylabel('Log Velocity (m/s)')
plt.ylim(0, 7)
plt.legend()
plt.tight_layout()
plt.savefig('step10_analyze_rotation/analyze_log_velo_with_log_angle_with_trendline_onlyLarge_nod16.pdf')
plt.savefig('step10_analyze_rotation/analyze_log_velo_with_log_angle_with_trendline_onlyLarge_nod16.png')
plt.show()
