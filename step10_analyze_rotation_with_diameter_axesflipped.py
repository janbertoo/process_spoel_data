...
# Calculate and print the average diameter of the dataset
average_diameter = total_rotation_df['diameter_median'].mean()
print(f"Average Diameter (m): {average_diameter:.3f}")

# Calculate and print the average rotation of the dataset
average_rotation = total_rotation_df['rotation'].mean()
print(f"Average Rotation (°): {average_rotation:.3f}")

# Plot the average diameter and rotation
plt.figure(figsize=(6, 4))
categories = ['Average Diameter', 'Average Rotation']
values = [average_diameter, average_rotation]
plt.bar(categories, values, color=['blue', 'orange'], alpha=0.7)
plt.ylabel('Value')
plt.title('Average Diameter and Rotation')
plt.tight_layout()

# Save the plot
plt.savefig('step10_analyze_rotation/average_diameter_rotation.png')
plt.savefig('step10_analyze_rotation/average_diameter_rotation.pdf')
...
#plt.title('Scatter plot of Diameter vs Rotation')
plt.xlabel('Diameter (m)')
plt.ylabel('Rotation (° / 0.25 s)')
plt.legend()
plt.tight_layout()
plt.savefig('step10_analyze_rotation/analyze_diameter_vs_rotation_with_trendline.png')
plt.savefig('step10_analyze_rotation/analyze_diameter_vs_rotation_with_trendline.pdf')
