...
# 3. Linear regression for the trendline
...
# 4. Define the bins for LSPI velocities
bins = np.arange(0, total_rotation_df['closest_gradient'].max() + 0.5, 0.5)
labels = [f"{i}-{i + 0.5}" for i in bins[:-1]]  # Create labels for the bins

# 5. Create a new column in the DataFrame for binned velocities
total_rotation_df['velocity_bins'] = pd.cut(total_rotation_df['closest_gradient'], bins=bins, labels=labels, right=False)

# 6. Aggregate the data to find average rotation per velocity bin
binned_data = total_rotation_df.groupby('velocity_bins')['abs_rotation'].mean().reset_index()

# 7. Plotting the binned data
plt.figure(figsize=(8, 5))  # Adjust figure size as necessary
