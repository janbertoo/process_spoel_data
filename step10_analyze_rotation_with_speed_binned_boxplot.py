...
# Aggregate the data to find average rotation per velocity bin
binned_data = total_rotation_df.groupby('velocity_bins')['abs_rotation'].mean().reset_index()

# Plotting the average rotation by velocity bins
plt.figure(figsize=(8, 5))
plt.bar(binned_data['velocity_bins'], binned_data['abs_rotation'], color='skyblue', width=0.4, alpha=0.7)
plt.title('Average Rotation by LSPI Velocity Bins')
plt.xlabel('LSPI Velocity (m/s)')
plt.ylabel('Average Rotation (°)')
plt.xticks(rotation=45)
plt.tight_layout()
plt.savefig('step10_analyze_rotation/average_rotation_by_velocity_bins.png')
plt.show()

# Create a boxplot for rotation values by velocity bins
plt.figure(figsize=(10, 6))
# Create the boxplot using the original DataFrame grouped by velocity bins
plt.boxplot([total_rotation_df[total_rotation_df['velocity_bins'] == label]['abs_rotation'] for label in labels],
            labels=labels, patch_artist=True, boxprops=dict(facecolor='lightblue'))
plt.title('Boxplot of Rotation by LSPI Velocity Bins')
plt.xlabel('LSPI Velocity Bins (m/s)')
plt.ylabel('Rotation (°)')
plt.xticks(rotation=45)
plt.ylim(0, 10)
plt.tight_layout()
plt.savefig('step10_analyze_rotation/boxplot_rotation_by_velocity_bins.png')
plt.show()
