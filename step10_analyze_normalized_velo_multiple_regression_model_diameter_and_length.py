...
# Drop any rows with missing values for length and diameter
total_velocity_df = total_velocity_df.dropna(subset=['normalised_velocity', 'length_median', 'diameter_median'])

# Build the multiple linear regression model without volume
X = total_velocity_df[['length_median', 'diameter_median']]  # Independent variables (without volume)
...
# Fit the model
model_without_volume = sm.OLS(y, X).fit()

# Print the summary of the regression
print(model_without_volume.summary())

# Plotting the residuals for a quick check of the model fit
plt.figure(figsize=(6, 4))
plt.scatter(model_without_volume.fittedvalues, model_without_volume.resid, alpha=0.6)
plt.axhline(0, color='red', linestyle='--')
plt.xlabel('Fitted values')
plt.ylabel('Residuals')
plt.title('Residuals vs Fitted Values (Without Volume)')
plt.tight_layout()
plt.show()
