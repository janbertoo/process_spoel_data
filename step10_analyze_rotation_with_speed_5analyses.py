...
# Plot 4: Boxplot of Velocity by Rotation Range
rotation_bins = pd.cut(var1, bins=np.arange(0, var1.max() + 10, 10))  # Binning rotation into intervals of 10
rotation_bins = pd.Categorical(rotation_bins)  # Ensure it's treated as categorical
plt.figure(figsize=(8, 4))
sns.boxplot(x=var2, y=rotation_bins, palette="Blues")  # Swap axes
plt.xticks(rotation=45)
plt.title('Boxplot of Rotation by Velocity Range')
plt.xlabel('LSPI Velocity (m/s)')
plt.ylabel('Rotation Range (°)')
plt.tight_layout()
plt.savefig('step10_analyze_rotation/boxplot_velocity_vs_rotation.png')
plt.show()

# Plot 5: Violin Plot of Velocity by Rotation Range
plt.figure(figsize=(8, 4))
sns.violinplot(x=var2, y=rotation_bins, palette="Blues")  # Swap axes
plt.xticks(rotation=45)
plt.title('Violin Plot of Rotation by Velocity Range')
plt.xlabel('LSPI Velocity (m/s)')
plt.ylabel('Rotation Range (°)')
plt.tight_layout()
plt.savefig('step10_analyze_rotation/violinplot_velocity_vs_rotation.png')
plt.show()
