...
# Analyze the correlation between normalized_velocity and log_angle
total_velocity_df = total_velocity_df.dropna(subset=['normalized_velocity', 'log_angle'])
...
# Pearson correlation
pearson_corr, pearson_p_value = stats.pearsonr(total_velocity_df['log_angle'], total_velocity_df['normalized_velocity'])
print(f"Pearson correlation coefficient: {pearson_corr:.3f}, P-value: {pearson_p_value:.3f}")

# Spearman correlation
spearman_corr, spearman_p_value = stats.spearmanr(total_velocity_df['log_angle'], total_velocity_df['normalized_velocity'])
print(f"Spearman correlation coefficient: {spearman_corr:.3f}, P-value: {spearman_p_value:.3f}")

# Linear regression
slope, intercept, r_value, p_value, std_err = stats.linregress(total_velocity_df['log_angle'], total_velocity_df['normalized_velocity'])
...
plt.legend()
plt.tight_layout()
plt.savefig('step10_analyze_rotation/analyze_normalized_velo_with_log_angle_with_trendline_onlyLarge_nod16.pdf')
plt.savefig('step10_analyze_rotation/analyze_normalized_velo_with_log_angle_with_trendline_onlyLarge_nod16.png')
plt.show()
