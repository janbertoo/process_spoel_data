...
from scipy.stats import gaussian_kde, normaltest, shapiro, anderson
...
# Step 5: Check normality of clusters
for cluster in range(num_clusters):
    subset = total_velocity_df[total_velocity_df['cluster'] == cluster]['flow_speed']
    print(f"Cluster {cluster} Normality Tests:")
    print(f"D'Agostino and Pearson's Test: {normaltest(subset)}")
    print(f"Shapiro-Wilk Test: {shapiro(subset)}")
    print(f"Anderson-Darling Test: {anderson(subset)}")
    print("\n")

# Step 6: Cluster-specific regression if Gaussian, else consider nonlinear regression
plt.figure(figsize=(6, 4))
colors = ['red', 'blue', 'green']
for cluster in range(num_clusters):
    subset = total_velocity_df[total_velocity_df['cluster'] == cluster]
    X = subset['log_angle'].values.reshape(-1, 1)
    y = subset['flow_speed'].values

    # If normality is confirmed, perform linear regression
    stat, p = normaltest(y)
    if p > 0.05:
        model = LinearRegression().fit(X, y)
        slope = model.coef_[0]
        intercept = model.intercept_
        print(f"Cluster {cluster} (Gaussian): Slope={slope:.4f}, Intercept={intercept:.4f}")
        ...
        plt.plot(x_vals, y_vals, color=colors[cluster], linestyle='dashed')
    else:
        print(f"Cluster {cluster} (Non-Gaussian): Consider nonlinear regression.")

plt.xlabel('Log Angle (degrees)')
plt.ylabel('Flow Velocity (m/s)')
plt.legend()
plt.title('Cluster-Specific Regression Based on Normality')
plt.show()
