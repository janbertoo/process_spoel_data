...
# Create equal-size bins for volume_median using qcut
volume_bins = pd.qcut(total_velocity_df['volume_median'], q=4, labels=['Low', 'Medium-Low', 'Medium-High', 'High'])

# Create a bar plot for 'log_angle' in steps of 10 degrees with different volume bins
plt.figure(figsize=(8, 6))

# Bin the log_angle values into intervals of 10 degrees
bins = np.arange(0, 100, 10)
total_velocity_df['angle_bin'] = pd.cut(total_velocity_df['log_angle'], bins=bins, right=False)

# Group the data by both angle_bin and volume_bins
grouped = total_velocity_df.groupby(['angle_bin', volume_bins]).size().unstack().fillna(0)

# Plot the grouped bar plot
grouped.plot(kind='bar', stacked=False, color=['#ff9999', '#66b3ff', '#99ff99', '#ffcc99'], edgecolor='black')

# Customize the plot
plt.title('Log Angle Distribution by Volume Bins (Equal Entries)')
plt.xlabel('Log Angle (degrees)')
plt.ylabel('Frequency')
plt.xticks(rotation=45)
plt.legend(title='Volume Bins')
plt.tight_layout()

# Save the plot
plt.savefig('log_angle_distribution_volume_bins_equal_entries.png')
plt.savefig('log_angle_distribution_volume_bins_equal_entries.pdf')
