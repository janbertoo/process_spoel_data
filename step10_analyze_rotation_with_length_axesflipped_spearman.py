...
spearman_corr, spearman_p_value = stats.spearmanr(total_rotation_df['length_median'], total_rotation_df['rotation'])
...
lowess = sm.nonparametric.lowess(total_rotation_df['rotation'], total_rotation_df['length_median'], frac=0.3)

# Print smallest and largest values of LOWESS trendline
min_lowess = np.min(lowess[:, 1])
max_lowess = np.max(lowess[:, 1])
print(f"LOWESS trendline range: Min = {min_lowess:.3f}, Max = {max_lowess:.3f}")

# Plotting
plt.figure(figsize=(4, 4))
plt.scatter(total_rotation_df['length_median'], total_rotation_df['rotation'], alpha=0.6, label='Data')
plt.plot(lowess[:, 0], lowess[:, 1], color='red', label=f'Spearman Trend (ρ: {spearman_corr:.3f}, P: {spearman_p_value:.3f})')
plt.xlabel('Length (m)')
plt.ylabel('Rotation (° / 0.25 s)')
plt.legend()
plt.tight_layout()
plt.savefig('step10_analyze_rotation/analyze_length_vs_rotation_spearman_trend.pdf')
plt.show()
