...
def read_detection_and_return_ID_plus_location(detections_path_drone1,detections_path_drone6,detections_path_drone16):
    ...
    try:
        with open(detections_path_drone1, 'r') as file:
            ...
            for line in lines:
                ...
                detections.append([ID,center,tl,tr,bl,br,x_center,y_center,drone_number])
    except:
        no_detections = True
    ...
    for detection in detections:
        ...
        for detection2 in detections:
            if detection[0] == detection2[0] and detection != detection2:
                double = True
                if detection[8] == 1:
                    if point_position_line1(detection[6],detection[7]) == "right":
                        detections_final.append(detection)
                if detection[8] == 6:
                    if point_position_line1(detection[6],detection[7]) == "left":
                        if point_position_line2(detection[6],detection[7]) == "right":
                            detections_final.append(detection)
                if detection[8] == 16:
                    if point_position_line2(detection[6],detection[7]) == "left":
                        detections_final.append(detection)
        if double == False:
            detections_final.append(detection)
...
def georectify_three_images(data_to_plot):
    #print(data_to_plot[0])
    data = data_to_plot[0]
    remembered_lines_georec = data_to_plot[1]
    ...
    if os.path.exists(output_image_path) != True:
        ...
        for remembered_lines_last_line in remembered_lines_last:
            for remembered_line_last in remembered_lines_last_line:
                x1,y1=remembered_line_last[0]
                x2,y2=remembered_line_last[1]
                plt.plot([x1, x2], [y1, y2], color='red', linestyle='-', linewidth=3)
...
if __name__ == "__main__":
    ...
    with multiprocessing.Pool(processes=num_processes) as pool:
        ...
        pool.map(georectify_three_images, data_to_plot)
