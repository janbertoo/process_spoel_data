...
#################################################################################################################################################################################
def compare_coordinates(original_XYZ_coordinates,original_UV_coordinates,homography):
    
    U = []
    V = []
    
    for coor in original_UV_coordinates:
        U.append(coor[0])
        V.append(coor[1])
    U = np.array(U)
    V = np.array(V)

    rectified_coordinates = rectify_coordinates(U,V,homography)
    
    #for i in range(len(rectified_coordinates)):
    #    print(original_XYZ_coordinates[i][0])
    #    print(rectified_coordinates[0][1])
    #    print(' ')

    #print(rectified_coordinates)
    #print(original_XYZ_coordinates)
    #print(' ')

    tot_diff = 0
    for i in range(len(original_XYZ_coordinates)):
        tot_diff = tot_diff + np.sqrt( ( original_XYZ_coordinates[i][0] - rectified_coordinates[0][i] ) ** 2 + ( original_XYZ_coordinates[i][1] - rectified_coordinates[1][i] ) ** 2 )

    average_diff = tot_diff / len(original_XYZ_coordinates)

    return(average_diff)

#for dronenumbervidnumber in [[1,1]]:
...
def process_viddrone(dronenumbervidnumber):
    ...
    homography_path = labels_with_gcps_path.replace('labels_with_gcps','homs_savgol_filtered')
    ...
    for entry in all_info:
        ...
        try:
            ...
            homography = np.array([[10,10,10],[10,10,10],[10,10,10]])
        #print(type(homography))
        score = compare_coordinates(XYZ,UV,homography)
        #print(score)
        ...
        count = 0
        diff_and_homs.append([score,totaldiff,count,drones_used,virtualGCPS_combos,homography,entry[4]])
        count = count + 1
        ...
        for i in range(tries_vir_GCPS):
            ...
            score = compare_coordinates(XYZ,UV,op_homography)
            ...
            diff_and_homs.append([score,op_totaldiff,count,drones_used,virtualGCPS_combos,op_homography,entry[4]])
            count = count + 1
        ...
        ######pickle_file = open(entry[4], 'wb')
        ######pickle.dump(diff_and_homs[0][4], pickle_file)

    #use savgol filter on all values of the homography
    average_over = 40 #frames
    filter_power = 2
    ...
    for values in all_data:
        #homography = all_homographies[0][5]
        #print(values[0][5])
        homography_0 = values[0][5][0][0]
        homography_series_0.append(homography_0)
        homography_1 = values[0][5][0][1]
        homography_series_1.append(homography_1)
        homography_2 = values[0][5][0][2]
        homography_series_2.append(homography_2)
        homography_3 = values[0][5][1][0]
        homography_series_3.append(homography_3)
        homography_4 = values[0][5][1][1]
        homography_series_4.append(homography_4)
        homography_5 = values[0][5][1][2]
        homography_series_5.append(homography_5)
        homography_6 = values[0][5][2][0]
        homography_series_6.append(homography_6)
        homography_7 = values[0][5][2][1]
        homography_series_7.append(homography_7)
        homography_8 = values[0][5][2][2]
        homography_series_8.append(homography_8)

        homography_p = values[0][6]
        #homography_series.append(homography)
        homography_paths.append(homography_p)

    homography_series_filtered_0 = savgol_filter(homography_series_0, average_over, filter_power)
    homography_series_filtered_1 = savgol_filter(homography_series_1, average_over, filter_power)
    homography_series_filtered_2 = savgol_filter(homography_series_2, average_over, filter_power)
    homography_series_filtered_3 = savgol_filter(homography_series_3, average_over, filter_power)
    homography_series_filtered_4 = savgol_filter(homography_series_4, average_over, filter_power)
    homography_series_filtered_5 = savgol_filter(homography_series_5, average_over, filter_power)
    homography_series_filtered_6 = savgol_filter(homography_series_6, average_over, filter_power)
    homography_series_filtered_7 = savgol_filter(homography_series_7, average_over, filter_power)
    homography_series_filtered_8 = savgol_filter(homography_series_8, average_over, filter_power)

    for o in range(len(homography_paths)):
        homography = np.array([[homography_series_filtered_0[o],homography_series_filtered_1[o],homography_series_filtered_2[o]],
            [homography_series_filtered_3[o],homography_series_filtered_4[o],homography_series_filtered_5[o]],
            [homography_series_filtered_6[o],homography_series_filtered_7[o],homography_series_filtered_8[o]]])
        pickle_file = open(homography_paths[o], 'wb')
        pickle.dump(homography, pickle_file)



    #plt.plot(all_framenumbers,all_homographies)
    #all_optimized_homographies_savgol_20_2 = savgol_filter(all_optimized_homographies, 20, 2)
    all_optimized_homographies_savgol_40_2 = savgol_filter(all_optimized_homographies, 40, 2)
    #all_optimized_homographies_savgol_60_2 = savgol_filter(all_optimized_homographies, 60, 2)
    #all_optimized_homographies_savgol_80_2 = savgol_filter(all_optimized_homographies, 80, 2)
    ax = plt.gca()
    ax.set_ylim([0, 30])
    #plt.savefig('homography_analysis/figure_drone'+str(dronenumber)+'_vid'+str(vidnumber)+'_Aoriginal.svg')
    plt.figure().set_figwidth(15)
    plt.plot(all_framenumbers,all_optimized_homographies, label="raw")
    #plt.plot(all_framenumbers[0:200],all_optimized_homographies_savgol_20_2[0:200], label="savgol 20 2")
    plt.plot(all_framenumbers,all_optimized_homographies_savgol_40_2, label="savgol 40 2")
    #plt.plot(all_framenumbers[0:200],all_optimized_homographies_savgol_60_2[0:200], label="savgol 60 2")
    #plt.plot(all_framenumbers[0:200],all_optimized_homographies_savgol_80_2[0:200], label="savgol 80 2")
