...
for vidnumber in vidnumbers:
    ...
    for i in range(len(df) - 1):
        ...
        if current_row['ID'] == next_row['ID'] and abs(current_row['timestamp'] - next_row['timestamp']) <= 0.3:
            ...
            mid_y = (current_row['center_geo_x'] + next_row['center_geo_y']) / 2
...
# Now, analyze the correlation between velocity_log and diameter_median
total_velocity_df = total_velocity_df.dropna(subset=['velocity_log', 'diameter_median'])

# Calculate and print diameter statistics
min_diameter = total_velocity_df['diameter_median'].min()
max_diameter = total_velocity_df['diameter_median'].max()
print(f"Smallest Diameter (m): {min_diameter:.3f}")
print(f"Largest Diameter (m): {max_diameter:.3f}")
...
# Linear regression
slope, intercept, r_value, p_value, std_err = stats.linregress(total_velocity_df['diameter_median'], total_velocity_df['velocity_log'])

# Plotting
plt.figure(figsize=(6, 4))
plt.scatter(total_velocity_df['diameter_median'], total_velocity_df['velocity_log'], alpha=0.6)
x_vals = np.array(plt.gca().get_xlim())
y_vals = intercept + slope * x_vals
plt.plot(x_vals, y_vals, color='red', label=f'Trend (Slope: {slope:.3f} \nP-value: {p_value:.3f})')
#plt.title('Diameter vs Log Velocity')
plt.xlabel('Diameter (m)')
plt.ylabel('Log Velocity (m/s)')
plt.ylim(0, 7)
plt.legend()
plt.tight_layout()
plt.savefig('step10_analyze_rotation/analyze_velocity_log_with_diameter_with_trendline_only_large.pdf')
plt.savefig('step10_analyze_rotation/analyze_velocity_log_with_diameter_with_trendline_only_large.png')
plt.show()
