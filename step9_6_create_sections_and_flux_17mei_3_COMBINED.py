...
for section, section_number, water_level_base, pre_section_file, post_section_file in sections_and_water_levels:
    ...
    num_subplots = 2#len(corrections_and_vidnumber) + 1  # +1 for the main plot
    ...
    ax.plot(df_post_section['distance_from_base'], df_post_section['Z'], color='b', label='Post Flood Section')
    ...
    ax2.bar(bin_centers, volume_sum, align='center', width=size_of_bins, color='brown')
    ax2.axvline(x=weighted_average, color='black', linestyle='--', label='Weighted Average',linewidth=2)
    
    ax2.set_ylabel('Volume (m^3)')
    ax2.legend()
    ax2.set_title(f'Video All, Total Volume {round(total_volume, 2)}')
    ...
    ax.axvline(x=weighted_average, color='black', linestyle='--', label=f'Correction',linewidth=2)
    
    plt.tight_layout()
    #plt.subplots_adjust(top=0.3, wspace=0.3)  # Adjust the top and wspace as needed
    #plt.show()
    plt.savefig(savefigfolder+'section_'+str(section_number)+'_wood_distribution_ALL_video.eps')
    plt.savefig(savefigfolder+'section_'+str(section_number)+'_wood_distribution_ALL_video.png')
    plt.savefig(savefigfolder+'section_'+str(section_number)+'_wood_distribution_ALL_video.pdf')
