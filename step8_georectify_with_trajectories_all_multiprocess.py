...
outputdirpath = '/media/jean-pierre/PortableSSD/work'
detections_path = 'interpolated_detected_linked_IDs_plus_orientation_coordinates-ce-tl-tr-bl-br_woodlength_wooddiameter_woodvolume'
        ...
        with open(detections_path_drone1, 'r') as file:
            ...
            for line in lines:
                ...
                if point_position_line1(x_center,y_center) == "right":
                    detections.append([ID,drone_number,center])
    except:
        no_detections = True
...
for i in range(len(stagered_drone_ims)):
    ...
    detectionspath_drone1 = os.path.join(data[3][4],'DCIM-drone1_drone1vid'+str(data[3][0])+'_frame'+str(data[3][1])+'.txt').replace('correct_homs_18jan',detections_path)
    detectionspath_drone6 = os.path.join(data[3][4],'DCIM-drone6_drone6vid'+str(data[3][0])+'_frame'+str(data[3][2])+'.txt').replace('correct_homs_18jan',detections_path)
    detectionspath_drone16 = os.path.join(data[3][4],'DCIM-drone16_drone16vid'+str(data[3][0])+'_frame'+str(data[3][3])+'.txt').replace('correct_homs_18jan',detections_path)

    #detections_drone1 = read_detection_and_return_ID_plus_location(detections_path_drone1,1)
    #detections_drone6 = read_detection_and_return_ID_plus_location(detections_path_drone6,6)
    #detections_drone16 = read_detection_and_return_ID_plus_location(detections_path_drone16,16)

    detections = read_detection_and_return_ID_plus_location(detectionspath_drone1,detectionspath_drone6,detectionspath_drone16)
    print(detectionspath_drone1)
    print(detectionspath_drone6)
    print(detectionspath_drone16)
    print(detections)
