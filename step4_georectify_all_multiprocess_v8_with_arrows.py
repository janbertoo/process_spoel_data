...
cmap = plt.get_cmap('viridis')  #coolwarm You can choose a different colormap

# Plot arrows using quiver
quiver = ax.quiver(X, Y, Vx, Vy, scale=1, scale_units='xy', angles='xy', color=cmap(normalized_arrow_lengths))
...
# Add a colorbar
#cbar = plt.colorbar(quiver, cax=cax, orientation='vertical', ticks=[0, 0.25, 0.5])
#cbar.set_label('Velocity Magnitude')

# Add a colorbar
#cbaxes = inset_axes(ax, width="30%", height="3%", loc=4) 
#cbar = plt.colorbar(cax=cbaxes,quiver, ax=ax)#, orientation="horizontal")
#cbar.set_label('Velocity Magnitude')
